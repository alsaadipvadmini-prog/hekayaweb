const fs = require('fs');
const glob = require('glob'); // Assuming glob is installed, if not we'll use fs.readdirSync recursively
...
for (const file of allFiles) {
  ...
  // Text that was white in dark sections
  content = content.replace(/text-white/gi, 'text-white'); // leave text-white alone if it's on a button, but need to be careful if it was on a dark section that is now light. Let's fix text manually if needed.
