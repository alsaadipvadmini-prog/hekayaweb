...
export const ProductCard: React.FC<ProductCardProps> = ({ product, index }) => {
  ...
  const isSolidButton = index % 3 === 0 || index % 5 === 0;
  ...
  return (
    <div
      ...
      className={\`group relative flex flex-col justify-between rounded-2xl p-4 transition-all duration-300 cursor-pointer border shadow-xl \${
        theme === 'dark'
          ? 'bg-white border-white/5 hover:border-neutral-300 hover:shadow-2xl hover:shadow-black'
          : 'bg-white border-neutral-200/90 hover:border-neutral-300 hover:shadow-xl hover:shadow-neutral-200/50'
      }\`}
    >
      ...
      <motion.div
        ...
        style={{
          backgroundColor:
            theme === 'dark'
              ? product.bgTint || 'var(--brand-cream)'
              : product.bgTint || 'var(--brand-cream)',
        }}
      >
        <img
        ...
        <div className="absolute top-3 w-full px-3 flex justify-between items-start pointer-events-none z-10">
          <div className="flex flex-col gap-1.5">
            {product.isClearance && (
              <span className="px-2.5 py-1 text-[9px] font-bold tracking-widest text-white bg-[#800020] rounded-sm shadow-sm uppercase font-mono">
                تصفية 1-5 د.أ
              </span>
            )}
            {product.badge && !product.isClearance && (
              <span className="px-2.5 py-1 text-[9px] font-bold tracking-widest text-white bg-[#120205] rounded-sm shadow-sm uppercase font-mono">
                {product.badge}
              </span>
            )}
          </div>
          ...
          <button
            ...
            className={\`pointer-events-auto p-2 rounded-full backdrop-blur-md shadow-xs transition-colors \${
              isWish ? 'bg-[#120205] text-white' : 'bg-white/80 text-neutral-400 hover:text-[#120205]'
            }\`}
      ...
      <div className="mt-4 flex flex-col flex-grow gap-2 px-1 text-center">
        <h3 className="font-bold text-sm md:text-base text-[#120205] line-clamp-2 leading-snug">
        ...
        <div className="flex items-center justify-center gap-2 mt-auto">
          ...
          <span className="text-base md:text-lg font-bold text-[#800020] font-mono">
            {product.price} د.أ
          </span>
        </div>
      </div>
      ...
      <div className="mt-4">
        {product.inStock ? (
          <button
            ...
            className={\`w-full py-3 rounded-xl text-xs font-bold transition-all duration-300 flex items-center justify-center gap-2 shadow-xs \${
              isSolidButton
                ? 'bg-[#120205] text-white hover:bg-[#3B020D]'
                : 'bg-white text-[#120205] border border-[#120205] hover:bg-neutral-50'
            }\`}
          >
